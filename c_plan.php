...
$html = file_get_html('something/union/c.html');
...
for ($i = 0; $i < 3; $i++) {
    $sql = "insert into c_plan values($monthly_fee[$i], $total_phone[$i], $flow[$i], $message, $video[$i],$phone_m[$i],$video_m,$flow_k,$message_m)";
    //加入数据库
    mysql_query($sql, $con);
}
